...
const ActiveExerciseView = ({
  ...
  const handleLog = () => {
    ...
    logWorkoutSet(exercise.id, weight, reps);
  ...
  return (
    <div className="absolute inset-0 flex flex-col bg-neutral-950 overflow-hidden">
                ...
                <div className="glass-panel p-4 rounded-xl space-y-2">
                  ...
                  {todayRecord.sets.map((set, i) => (
                    <div key={set.id} className="flex items-center justify-between bg-neutral-900/50 rounded-lg px-3 py-2 text-sm">
                      <span className="font-bold text-white">{i+1}. {set.weight}kg × {set.reps}次</span>
              ...
              <div className="glass-panel p-4 rounded-xl">
                ...
                <div className="flex gap-2 h-16">
                  <div className="flex-1 bg-neutral-900 rounded-xl flex border border-neutral-800">
                    ...
                    <input type="number" value={inputCache.reps} onChange={e => onInputChange('reps', e.target.value)} className="w-full bg-transparent text-center text-xl font-black text-white focus:outline-none" placeholder="次" />
                  </div>
                  <button onClick={handleLog} disabled={!inputCache.weight || !inputCache.reps} className="w-16 bg-primary text-white rounded-xl flex items-center justify-center disabled:opacity-20 shadow-lg active:scale-95 transition-all">
      ...
      <div className="shrink-0 px-4 py-4 border-t border-white/5 bg-neutral-950/80 backdrop-blur-md flex gap-3 z-30">
        <button onClick={onPrev} disabled={index === 0} className="w-12 h-12 rounded-xl border border-white/10 flex items-center justify-center text-neutral-400 disabled:opacity-20 btn-scale"><ChevronLeft size={22} /></button>
        
        <button
          onClick={() => setShowInsertPicker(true)}
          className="flex items-center justify-center w-12 h-12 rounded-xl border border-primary/30 bg-primary/10 text-primary transition-all active:scale-95 btn-scale"
          title="插入下一个动作"
        >
          <Plus size={22} />
        </button>

        <button onClick={() => { if(isLast) playSuccessSound(); onNext(); }} className={`flex-1 h-12 rounded-xl font-bold flex items-center justify-center gap-2 ${isLast ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-primary/20 text-primary border border-primary/30'}`}>
